...
def read_step_file(step, d):
    graphs = []
    with open("step%d_d=%d.txt"%(step, d), "r") as f:
        A = []
...
if os.path.exists("step%d_d=%d.txt"%(1, n)):
    graphs = read_step_file(1, n)
else:
    graphs = gc.Step1(n)
print("step 1: ")
with open("step1_d=%d.txt"%n,"w") as f:
    for g in graphs:
        for line in g:
...
print("step 2: ")
with open("step2_d=%d.txt"%n,"w") as f:
...
print("step 3: ")
with open("step3_d=%d.txt"%n,"w") as f:
...
print("step 4: ")
with open("step4_d=%d.txt"%n,"w") as f:
...
print("step 5: ")
with open("step5_d=%d.txt"%n,"w") as f:
...
plt.savefig("graph_growth_rate_d=%d.png"%n)
plt.clf()

cs = [len(x.split('_')) for x in G.nodes]
nx.draw(G, node_size=20, node_color=cs)
plt.savefig("graph_of_connections_d=%d.png"%n)
